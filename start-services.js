...
function checkAndInstallDependencies() {
  ...
  for (const service of services) {
    ...
    if (!fs.existsSync(nodeModulesPath)) {
      console.log(`${service.color}[${service.name}]${resetColor} Installing dependencies...`);
      try {
        execSync('npm install', { 
          cwd: serviceDir, 
          stdio: 'inherit' 
        });
        console.log(`${service.color}[${service.name}]${resetColor} Dependencies installed successfully`);
      } catch (error) {
        console.error(`${service.color}[${service.name}]${resetColor} Failed to install dependencies: ${error.message}`);
      }
    } else {
      console.log(`${service.color}[${service.name}]${resetColor} Dependencies already installed`);
    }
  }
  console.log('');
}
...
function startService(service) {
  ...
  if (!fs.existsSync(serviceDir)) {
    console.error(`${service.color}[${service.name}]${resetColor} Direktori layanan tidak ditemukan: ${serviceDir}`);
    return;
  }
  ...
  if (!fs.existsSync(packageJsonPath)) {
    console.error(`${service.color}[${service.name}]${resetColor} File package.json tidak ditemukan di: ${packageJsonPath}`);
    return;
  }
  
  console.log(`${service.color}[${service.name}]${resetColor} Memulai layanan pada port ${service.port}...`);
  ...
  try {
    ...
    serviceProcess.stderr.on('data', (data) => {
      ...
      lines.forEach(line => {
        if (line.trim()) {
          console.error(`${service.color}[${service.name}]${resetColor} ${line.trim()}`);
        }
      });
    });
    ...
    serviceProcess.on('close', (code) => {
      console.log(`${service.color}[${service.name}]${resetColor} proses berhenti dengan kode: ${code}`);
    });
    
    // Tangani error
    serviceProcess.on('error', (err) => {
      console.error(`${service.color}[${service.name}]${resetColor} error: ${err.message}`);
    });
    
    return serviceProcess;
  } catch (error) {
    console.error(`${service.color}[${service.name}]${resetColor} Gagal memulai layanan: ${error.message}`);
    return null;
  }
}
...
process.on('SIGINT', () => {
  ...
  serviceProcesses.forEach((proc, index) => {
    if (proc) {
      ...
      console.log(`${service.color}[${service.name}]${resetColor} Menghentikan layanan...`);
...
console.log('\x1b[1mTambahkan script berikut ke package.json root untuk mempermudah:\x1b[0m');
console.log(`
"scripts": {
...
services.forEach(service => {
  console.log(`${service.color}[${service.name}]${resetColor} http://localhost:${service.port}`);
});

console.log('\n\x1b[1m===== ENDPOINT API UTAMA =====\x1b[0m');
console.log('\x1b[32m[production_management]\x1b[0m http://localhost:5001/api/batches');
console.log('\x1b[33m[production_planning]\x1b[0m http://localhost:5002/api/requests');
console.log('\x1b[35m[machine_queue]\x1b[0m http://localhost:5003/api/queue');
console.log('\x1b[34m[material_inventory]\x1b[0m http://localhost:5004/api/inventory');
console.log('\x1b[31m[production_feedback]\x1b[0m http://localhost:5005/api/feedback');
