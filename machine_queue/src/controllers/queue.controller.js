  ...
  try {
    ...
    if (!machineId || !batchId || !batchNumber || !productName || !hoursRequired) {
      return res.status(400).json({
        success: false,
        message: 'Required fields: machineId, batchId, batchNumber, productName, hoursRequired'
      });
    }
    ...
    const newQueue = await MachineQueue.create({
      ...
      step_id: stepId,
      step_name: stepName,
      scheduled_start_time: scheduledStartTime ? new Date(scheduledStartTime) : null,
...
exports.updateQueue = async (req, res) => {
  try {
    ...
    const allowedFields = [
      'scheduledStartTime', 'scheduledEndTime', 'hoursRequired',
      'priority', 'operatorId', 'operatorName', 'setupTime', 'notes'
    ];
...
exports.reorderMachineQueue = async (machineId) => {
  try {
    const waitingQueues = await MachineQueue.findAll({
      ...
      order: [['position', 'ASC'], ['priority', 'DESC'], ['createdAt', 'ASC']]
    });
