      ...
      include: [
        {
          model: Material,
          as: 'materials',
          attributes: ['id', 'materialId', 'name', 'category', 'type', 'stockQuantity', 'unit', 'price']
        }
      ]
    });
...
exports.createSupplier = async (req, res) => {
  try {
    ...
    const newSupplierId = supplierId || `SUP-${Date.now().toString().slice(-6)}`;
  ...
  try {
    ...
    const suppliers = await Supplier.findAll({
      ...
      attributes: [
        ...
        'rating', 
        'leadTime',
        [db.sequelize.fn('COUNT', db.sequelize.col('materials.id')), 'materialCount']
      ],
      group: ['Supplier.id'],
      order: [['rating', 'DESC']]
    });
