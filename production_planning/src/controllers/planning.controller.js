  ...
  try {
    ...
    try {
      const response = await axios.get(
        `${process.env.PRODUCTION_MANAGEMENT_URL}/api/production/${requestId}`
      );
...
exports.createPlan = async (req, res) => {
  try {
    ...
    if (!requestId || !productName) {
      return res
        .status(400)
        .json({ message: "ID permintaan dan nama produk diperlukan" });
    }
...
exports.updatePlan = async (req, res) => {
  try {
    ...
    if (totalCapacityRequired)
      updateData.totalCapacityRequired = totalCapacityRequired;
...
exports.approvePlan = async (req, res) => {
  try {
    ...
    if (!plan.capacityPlans || plan.capacityPlans.length === 0) {
      return res
        .status(400)
        .json({ message: "Rencana kapasitas belum dibuat" });
    }
...
exports.addCapacityPlan = async (req, res) => {
  try {
    ...
    if (!machineType || !hoursRequired) {
      return res
        .status(400)
        .json({ message: "Tipe mesin dan jam yang dibutuhkan harus diisi" });
    }
...
exports.addMaterialPlan = async (req, res) => {
  try {
    ...
    if (!materialId || !materialName || !quantityRequired || !unitOfMeasure) {
      return res
        .status(400)
        .json({ message: "ID material, nama, jumlah, dan satuan harus diisi" });
    }

    // Hitung total biaya jika ada biaya satuan
    const totalCost = unitCost
      ? parseFloat(unitCost) * parseFloat(quantityRequired)
      : null;
    ...
    try {
      const response = await axios.post(
        `${process.env.MATERIAL_INVENTORY_URL}/api/inventory/check`,
        {
          materialId,
      ...
      await materialPlan.update({
        ...
        notes: response.data.available
          ? "Material tersedia"
          : `${notes || ""}\nPeringatan: ${
              response.data.message || "Material mungkin tidak tersedia"
            }`,
      });
    } catch (error) {
...
async function planProduction(planId) {
  try {
    ...
    try {
      const response = await axios.get(
        `${process.env.PRODUCTION_MANAGEMENT_URL}/api/production/${plan.requestId}`
      );
      request = response.data;
    } catch (error) {
      console.error("Gagal mendapatkan detail permintaan:", error.message);
    ...
    try {
      const response = await axios.get(
        `${
          process.env.MATERIAL_INVENTORY_URL
        }/api/materials/product?name=${encodeURIComponent(plan.productName)}`
      );
    ...
    try {
      const response = await axios.get(
        `${
          process.env.MACHINE_QUEUE_URL
        }/api/machines/product?name=${encodeURIComponent(plan.productName)}`
      );
    ...
    for (const material of materials) {
      ...
      // Hitung total biaya
      const totalCost = material.unitCost
        ? parseFloat(material.unitCost) * totalRequired
        : 0;
      ...
      try {
        ...
        console.error(
          `Gagal memeriksa ketersediaan material ${material.name}:`,
          error.message
        );
      }
    }
    ...
    for (const machine of machines) {
      ...
      try {
        ...
        console.error(
          `Gagal memeriksa ketersediaan kapasitas untuk ${machine.type}:`,
          error.message
        );
      }
    }
    ...
    console.log(
      `Perencanaan otomatis untuk rencana ${plan.planId} berhasil selesai`
    );
  } catch (error) {
