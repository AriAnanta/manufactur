...
const root = {
  ...
  // Production Request Mutations
  createProductionRequest: async ({ input }) => {
    try {
  ...
  createProductionBatch: async ({ input }) => {
    try {
      ...
      const newBatch = await ProductionBatch.create({
        batchNumber,
        request_id: request.requestId, // Menggunakan request.requestId (string) bukan input.requestId (integer)
        quantity: input.quantity,
      ...
      const response = await axios.post(`${process.env.PRODUCTION_REQUEST_SERVICE??"https://b7b3klvk-4.asse.devtunnels.ms/graphql"}`, 
        {
          query: `
          mutation updateRequest($payload: UpdateProductionRequestInput!){
              updateProductionRequest(input: $payload) {
                id
                product_id
                product_name
                quantity
          ...
          variables: {
            payload: {
              id: request.requestId ?? input.requestId,
              manufacturer_batch_id: batchNumber
            }
          }
        }
      )
